...
class ExecutorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fast_trace_is_transition_based(self) -> None:
        ha = FakeHa()
        ...
        confirmed = await executor.reconcile(
            [desired],
            {
                "climate.fast": actual(
                    "climate.fast",
        ...
        self.assertEqual(0, confirmed.commands)
        self.assertTrue(
            any(
                title == "FAST · livingroom" and "desired=mode=heat target=22.0" in message
    ...
    async def test_retry_is_bounded_and_enters_cooldown(self) -> None:
        ha = FakeHa()
        climate_log = FakeClimateLog()
        ...
        cooldown = await executor.reconcile([desired], states, now_monotonic=106)
        ...
        self.assertEqual("no_confirmation", second.problems[0].reason)
